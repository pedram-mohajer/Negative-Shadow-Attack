...
with torch.no_grad():
    ...
    for i in range(seg.size(0)):
        ...
        for seg_class_index, seg_mask in enumerate(seg_mask_list):
            ...
            seg_filename = f'{output}/{os.path.basename(img_path[i])}_{params.seg_list[seg_class_index]}' if seg_mode == MULTILABEL_MODE else \
                           f'{output}/{os.path.basename(img_path[i])}'
    ...
    for i in range(len(ori_imgs)):
        ...
        if show_det:
            cv2.imwrite(f'{output}/{img_path[i]}_det.jpg',  cv2.cvtColor(det_only_imgs[i], cv2.COLOR_RGB2BGR))
